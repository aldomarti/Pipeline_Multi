...
- Descripción        : Job de carga archivos json verint sentiments
...
key_mapping = {
    ...
    "Fecha_ini_llamada":"Fecha_ini_llamada",
    "Sentiment_Score":"Sentiment_Score",
    "Sentiment":"Sentiment",
    "Remote_Player_URL":"Remote_Player_URL"
}
...
if P_FECHA_INI == '2020-01-01':
    # Sentimets ejecuta siempre con d-2 a d-1
    start_date = today_date.date() - datetime.timedelta(days=2)
    end_date = today_date.date()- datetime.timedelta(days=1) 
else:
...
def process_day(date):
    ...
    ruta = f"modelosentimentibk/{date.strftime('%Y-%m-%d')}/"
    ...
    for page in page_iterator:
        if 'Contents' in page:
            for obj in page['Contents']:
                if obj['Key'] == f"modelosentimentibk/{date.strftime('%Y-%m-%d')}/":
                    pass
                else:
                    try:
    ...
    if df.empty:
        ...
        s3_key = (
            f"pre-stage/motion/streams/hubclientes/t_verint_sentiment/"
        ...
        athena_table = 't_verint_sentiment'
